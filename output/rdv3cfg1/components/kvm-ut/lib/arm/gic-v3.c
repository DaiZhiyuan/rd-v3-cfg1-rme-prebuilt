...
void gicv3_enable_defaults(void)
{
	...
	/*
	 * If current GIC is capable of triggering vSGI directly from ITS,
	 * we'll need to enable nASSGIreq[8] which will request host to disable
	 * active state of vSGIs. This is required to benefit from direct vSGI
	 * injection feature introduced in GICv4.1.
	 */
	if (readl(dist + GICD_TYPER2) & GICD_TYPER2_nASSGIcap) {
		val |= GICD_CTLR_nASSGIreq;
		writel(val, dist + GICD_CTLR);
	}

	/*
	 * There is no need to perform a RWP[31] (Register Write Pending) wait
	 * after updating nASSGIreq[8] since, this bit field is not tracked by
	 * RWP[31] of GICD_CTLR register.
	 */
	val = readl(dist + GICD_CTLR);
